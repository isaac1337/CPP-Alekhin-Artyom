...
class NoCommand : public Command { public: void execute() override {} };
...
class RotateCommand : public Command
{
    ...
    float      mAngle;
public:
    RotateCommand(ShapePool& p, size_t idx, float a) : mPool(p), mIndex(idx), mAngle(a) {}
...
class RandomColorCommand : public Command
{
    ...
    size_t     mIndex;
public:
    RandomColorCommand(ShapePool& p, size_t idx) : mPool(p), mIndex(idx) {}
    void execute() override { mPool.getShape(mIndex).setFillColor(getRandomColor()); }
};
...
class AddNewRandomShape : public Command
{
    ...
    float      mMaxSize;
public:
    AddNewRandomShape(ShapePool& p, float mn, float mx) : mPool(p), mMinSize(mn), mMaxSize(mx) {}
    void execute() override
    {
        int r = getRandomInt(0, 2);
        sf::Shape* ps;
        if (r == 0)
            ps = new sf::RectangleShape({getRandomFloat(mMinSize, mMaxSize),
                                         getRandomFloat(mMinSize, mMaxSize)});
        else if (r == 1)
            ps = new sf::CircleShape(getRandomFloat(mMinSize / 2, mMaxSize / 2));
        else
            ps = new sf::CircleShape(getRandomFloat(mMinSize / 2, mMaxSize / 2), 3);
...
class RandomAllColorsCommand : public Command
{
    ShapePool& mPool;
public:
    explicit RandomAllColorsCommand(ShapePool& p) : mPool(p) {}
    void execute() override
    {
        for (size_t i = 0; i < mPool.getNumbeorOfShapes(); ++i)
...
int main()
{
    ...
    cp.addButton(new Button(wnd, {40, 380, 240, 40}, font, "All Random Colors"),     new RandomAllColorsCommand(pool)); // новая кнопка
    cp.addButton(new Button(wnd, {40, 440, 240, 40}, font, "Undo"));
